...
// --- 衝突檢測輔助函式 ---
const detectDuplicates = (scheduleData) => {
    const conflicts = [];

    // 為每個週的班表檢查重複排班
    scheduleData.forEach((weekData, weekIndex) => {
        const { schedule, scheduleDays } = weekData;
        ...
        schedule.forEach((daySchedule, dayIndex) => {
            ...
            // 收集當天所有已排班的人員
            const personnelThisDay = new Set();
            const duplicates = new Set();

            daySchedule.forEach((taskPersonnel) => {
                taskPersonnel.forEach((personName) => {
                    if (personName) {
                        if (personnelThisDay.has(personName)) {
                            duplicates.add(personName);
                        }
                        personnelThisDay.add(personName);
                    }
                });
            });

            // 如果有重複，記錄衝突
            duplicates.forEach(personName => {
                conflicts.push({
                    type: 'duplicate',
                    person: personName,
                    week: weekIndex + 1,
                    date: scheduleDays[dayIndex].date
                });
            });
        });
    });
...
const detectOverwork = (scheduleData, maxConsecutiveDays = 5) => {
    const conflicts = [];
    const personnelWorkDays = new Map(); // personName -> array of { date, weekIndex, dayIndex }
    ...
    scheduleData.forEach((weekData, weekIndex) => {
        ...
        schedule.forEach((daySchedule, dayIndex) => {
            ...
            const currentDate = scheduleDays[dayIndex].date;

            daySchedule.forEach((taskPersonnel) => {
                taskPersonnel.forEach((personName) => {
                    if (personName) {
                        if (!personnelWorkDays.has(personName)) {
                            personnelWorkDays.set(personName, []);
                        }
                        personnelWorkDays.get(personName).push({
                            date: currentDate,
                            weekIndex,
                            dayIndex
                        });
    ...
    // 檢查每位員工的連續工作天數
    personnelWorkDays.forEach((workDays, personName) => {
        // 排序日期（按照日期字串排序）
        ...
        let consecutiveStart = uniqueWorkDays[0]?.date;
        const consecutiveDates = [uniqueWorkDays[0]?.date];

        for (let i = 1; i < uniqueWorkDays.length; i++) {
            const prevDate = uniqueWorkDays[i - 1].date;
            const currDate = uniqueWorkDays[i].date;

            // 檢查日期是否連續（考慮週末和假日）
            const prevDateObj = new Date(
                parseInt(prevDate.substring(0, 4)),
            ...
            // 如果相差 1-3 天（允許週末），視為連續
            // 1天 = 連續工作日
            // 2天 = 中間有1天假期
            // 3天 = 週五到週一（中間是週末）
            if (daysDiff >= 1 && daysDiff <= 3) {
                consecutiveCount++;
                consecutiveDates.push(currDate);

                // 如果超過最大連續天數，記錄衝突（只記錄一次）
                if (consecutiveCount > maxConsecutiveDays && consecutiveCount === maxConsecutiveDays + 1) {
                    conflicts.push({
                        type: 'overwork',
                        person: personName,
...
app.post('/api/schedules/validate', (req, res) => {
    try {
        ...
        // 設定預設限制條件
        const maxConsecutiveDays = constraints?.maxConsecutiveDays || 5;
        const minRestDays = constraints?.minRestDays || 2;

        // 執行衝突檢測
        const duplicateConflicts = detectDuplicates(schedule);
        const overworkConflicts = detectOverwork(schedule, maxConsecutiveDays);

        // 合併所有衝突
        const allConflicts = [...duplicateConflicts, ...overworkConflicts];
        ...
        debugSchedule('驗證班表時發生錯誤:', error);
        res.status(500).json({ message: '驗證班表時發生未預期的錯誤' });
    }
});
